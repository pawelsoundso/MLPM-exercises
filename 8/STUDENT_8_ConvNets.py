...
# +
from keras.layers import Conv2D, MaxPooling2D, Dense, Flatten, GlobalMaxPool2D, Dropout, Activation
...
def get_fully_connected(input_shape=(2352,), n_classes=8):
    ...
    #ridgemodel.add(Dense(8, activation='softmax', input_shape=(2352,), kernel_regularizer=regularizers.l2(0.001))) 
    model.add(Dense(200, activation='relu', input_shape=(2352,), kernel_regularizer=regularizers.l2(0.001)))
    model.add(Dense(100, activation='relu', input_shape=(2000,), kernel_regularizer=regularizers.l2(0.001)))
    model.add(Dense(8, activation='softmax', input_shape=(1000,), kernel_regularizer=regularizers.l2(0.001)))
...
def get_cnn(input_shape=(28,28,3), n_classes=8):
    ...
    model.add(Conv2D(12, 5, 5, activation = 'relu', input_shape=input_shape, init='he_normal'))
...
def get_cnn4Comp(input_shape=(28,28,3), n_classes=8):
    ...
    model.add(Conv2D(filters=96, input_shape=input_shape, kernel_size=(3,3), strides=(1,1), padding='valid'))
    model.add(Activation('relu'))
    # Max Pooling
    model.add(MaxPooling2D(pool_size=(2,2), strides=(1,1), padding='valid')) # stride orig: 2
    ...
    model.add(Dense(4096, input_shape=(input_size,)))
    model.add(Activation('relu'))
...
comp_model.compile(optimizer=Adam(lr=0.0005), loss=categorical_crossentropy, metrics=['accuracy'])
comp_model.fit(X_train_rs, y_train, epochs=100, batch_size=128, validation_data=(X_valid_rs, y_valid), shuffle=True) 
