...
def create_placeholders(n_x, n_y):
    ...
    X = tf.placeholder(tf.float32, name='X', shape=[n_x, None]) #your_code
    Y = tf.placeholder(tf.float32, name='Y', shape=[n_y, None]) #your_code
...
def initialize_parameters(architecture):
    ...
    for i, param in enumerate(architecture):
        ...
        input_dim = param['input_dim'] #your_code
        output_dim = param['output_dim'] #your_code
...
def forward_propagation(X, architecture, parameters):
    ...
    for i, layer_parameters in enumerate(parameters):
        ...
        Z = tf.add(tf.matmul(layer_parameters['weights'], A), layer_parameters['bias']) #your_code
...
def compute_loss(Y, Z, activation='sigmoid'):
    ...
    if activation == 'sigmoid':
        loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits, labels=labels)) #your_code
    elif activation == 'softmax':
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=labels)) #your_code
    else:
...
def model(X_train, Y_train, X_test, Y_test, architecture, learning_rate = 0.0001,
    ...
    parameters = initialize_parameters(architecture) #your_code
    
    # Forward propagation: Build the forward propagation in the tensorflow graph with our function from above
    Z_n = forward_propagation(X, architecture, parameters) #your_code
    
    # Loss function: Add loss function to tensorflow graph
    loss = compute_loss(Y, Z_n, architecture[-1]['activation']) #your_code
    ...
    with tf.Session() as sess:
        ...
        accuracy = tf.reduce_mean(tf.cast(correct_prediction, 'float')) #your_code
